...
public class Matrix {
     ...
     */
    private void backwardStep(double[][] matrix, double[] vector, double[] result) {
    ...
    public double[][] invert() {
        ...
        for (int i = 0; i < n; i++) {
            double[] vector = new double[n];
            vector[i] = 1;
            // Находим решение для i столбца
            double[] roots = solve(matrix, vector);
            // Потом переносим корни в матрицу конечную
            for (int r = 0; r < n; r++) {
                result[r][i] = roots[r];
            }
        }
    ...
    public double[] solve(double[][] inputMatrix, double[] inputVector) {
        double[] result = new double[inputVector.length];
        double[][] matrix = copyMatrix(inputMatrix);
        double[] vector = new double[inputVector.length];
        System.arraycopy(inputVector, 0, vector, 0, n);

        forwardStep(matrix, vector);
        backwardStep(matrix, vector, result);
        ...
        for (int m = 0; m<n; m++) {
            for (int i = 0; i<=n; i++) {
                if (i<n) System.out.format("%10.3f", matrix[m][i] );
                    ...
                    if (vector != null) System.out.format(" : %8.3f", vector[m]);
                }
            }
            System.out.println();
        }
